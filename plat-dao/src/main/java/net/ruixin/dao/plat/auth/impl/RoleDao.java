...
public class RoleDao extends BaseDao<SysRole> implements IRoleDao {
    ...
    @Override
    public Map<String, Object> getGlxxByRole(Long roleId) {
        Map<String, Object> temp;
        List<Map<String, Object>> users;
        List<Map<String, Object>> organs = new ArrayList<>();
        Map<String, List<Map<String, Object>>> include = new HashMap<>();
        Map<String, List<Map<String, Object>>> exclude = new HashMap<>();
        String sql = "SELECT GR.ID,\n" +
                ...
                "         WHEN '3' THEN\n" +
                "          (SELECT ORGAN_ID\n" +
                "             FROM SYS_GLB_ORGAN_USER_POST\n" +
                "            WHERE USER_ID = GR.GL_ID\n" +
                "              AND SFYX_ST = '1')\n" +
                "         ELSE\n" +
                "          NULL\n" +
                "       END) ORGAN_ID\n" +
                "  FROM SYS_GLB_ROLE GR\n" +
                " WHERE GR.ROLE_ID = ? AND GR.SFYX_ST='1' ";
        List<Map<String, Object>> glxxs = super.getJdbcTemplate().queryForList(sql, roleId);
        for (Map glxx : glxxs) {
            if ("2".equals(glxx.get("GL_TYPE"))) {
                temp = new HashMap<>();
                temp.put("id", glxx.get("ID"));
                temp.put("glId", glxx.get("GL_ID"));
                temp.put("lx", "jg");
                organs.add(temp);
            }
            if ("3".equals(glxx.get("GL_TYPE"))) {
                if ("1".equals(glxx.get("SFQY_ST"))) {
                    temp = new HashMap<>();
                    temp.put("id", glxx.get("ID"));
                    temp.put("glId", glxx.get("GL_ID"));
                    temp.put("lx", "ry");
                    if (include.containsKey(glxx.get("ORGAN_ID") + "jg")) {
                        include.get(glxx.get("ORGAN_ID") + "jg").add(temp);
                    } else {
                        users = new ArrayList<>();
                        users.add(temp);
                        include.put(glxx.get("ORGAN_ID") + "jg", users);
                    }
                    ...
                    temp.put("glId", glxx.get("GL_ID"));
                    temp.put("lx", "ry");
                    if (exclude.containsKey(glxx.get("ORGAN_ID") + "jg")) {
                        exclude.get(glxx.get("ORGAN_ID") + "jg").add(temp);
                    } else {
                        users = new ArrayList<>();
                        users.add(temp);
                        exclude.put(glxx.get("ORGAN_ID") + "jg", users);
                    }
                }
            }
        }
        ...
        result.put("inUsers", include);
        result.put("outUsers", exclude);
        return result;
    }

    @Override
    public void saveRoleGlxx(Object[] params) {
        super.prepareCallNoReturn("{call PKG_PLATFORM.USP_SAVE_ROLE(?,?,?,?,?,?,?,?,?)}", params);
    }

    @Override
    public List<Map<String, Object>> getRuleByRole(Long roleId) {
        String sql = "select g.id, g.role_id as roleId, g.RULE_ID as ruleId, " +
