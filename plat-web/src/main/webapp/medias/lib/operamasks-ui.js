(function ($, undefined) {
    ...
    $.fn.extend({
        propAttr: $.fn.prop || $.fn.attr, _oldFocus: $.fn.focus, focus: function (delay, fn) {
...
(function ($, undefined) {
    ...
    var horizontalPositions = /left|center|right/, verticalPositions = /top|center|bottom/, center = "center",
        _position = $.fn.position, _offset = $.fn.offset;
...
(function ($, undefined) {
    ...
    $.om.plugin.add("omDraggable", "scroll", {
        onStart: function (ui, event) {
            ...
            var i = $(this).data("omDraggable"), o = i.options, scrolled = false, scrollSensitivity = 20,
                scrollSpeed = 20;
...
(function ($, undefined) {
    $.omWidget("om.omResizable", $.om.omMouse, {
        ...
        _create: function () {
            ...
            this.handles = o.handles || (!$('.om-resizable-handle', this.element).length ? "e,s,se" : {
                n: '.om-resizable-n',
                e: '.om-resizable-e',
                s: '.om-resizable-s',
                w: '.om-resizable-w',
        ...
        _mouseStop: function (event) {
            ...
            if (this._helper) {
                var pr = this._proportionallyResizeElements, ista = pr.length && (/textarea/i).test(pr[0].nodeName),
                    soffseth = ista && self._hasScroll(pr[0], 'left') ? 0 : self.sizeDiff.height,
    ...
    $.om.plugin.add("omResizable", "alsoResize", {
        start: function (event, ui) {
            ...
            var self = $(this).data("omResizable"), o = self.options, os = self.originalSize,
        ...
        start: function (event, ui) {
            ...
            if (/document/.test(oc) || oc == document) {
                ...
                var co = self.containerOffset, ch = self.containerSize.height, cw = self.containerSize.width,
                    width = (self._hasScroll(ce, "left") ? ce.scrollWidth : cw),
                    height = (self._hasScroll(ce) ? ce.scrollHeight : ch);
                self.parentData = {element: ce, left: co.left, top: co.top, width: width, height: height};
            }
        }, resize: function (event, ui) {
            var self = $(this).data("omResizable"), o = self.options, ps = self.containerSize,
            ...
            var self = $(this).data("omResizable"), o = self.options, cp = self.position, co = self.containerOffset,
...
(function ($) {
    ...
    $.omWidget("om.omPanel", {
        options: {
            ...
            var that = this, options = this.options, tools = options.tools,
            ...
            var options = this.options, $body = this.element, $loadMsg = $body.next(".om-panel-loadingMessage"),
                position = {
            ...
            var self = this, $body = this.element, $header = $body.prev(), $parent = $body.parent(),
                $loadMessage = $body.next(".om-panel-loadingMessage"), options = this.options, anim = effects.anim,
                speed = effects.speed;
    ...
    $.omWidget("om.omAccordion", {
        options: {
            ...
            if (($disableDiv = $acc.find(">.om-accordion-disable")).length === 0) {
                $("<div class='om-accordion-disable'></div>").css({
                    position: "absolute",
                    top: 0,
                    left: 0
            ...
            var $acc = this.element, panels = $.data(this.element, "panels"),
                panel = $acc.children().find(">div#" + index), len = panels.length, oldIndex = index;
            ...
            var $acc = this.element, self = this, panels = [], options = this.options,
                $headers = $acc.find(options.header), cfg = [], first;
            $headers.find("a").each(function (n) {
                ...
                var $anchor = $(this);
                cfg[n] = {
                    title: $anchor.text(), iconCls: $anchor.attr("iconCls"), onExpand: function (event) {
            ...
            var panels = $.data(this.element, "panels"), len = panels.length, options = this.options, self = this,
                isExpand = false, expandIndex = -1, anim = false, speed;
...
(function ($) {
    $.omWidget('om.omButton', {
        options: {disabled: null, label: null, icons: {left: null, right: null}, width: null, onClick: null},
        _create: function () {
            ...
            }).wrap(wrapperSpan).before(leftSpan).after(rightSpan).wrap(centerSpan);
        },
        _init: function () {
        ...
        _initButton: function () {
            ...
            if (options.icons.right) {
                if (label != '') {
                    $('<img>').attr('src', options.icons.right).css({
...
(function ($) {
    $.omWidget("om.omCalendar", {
        options: {
            ...
            var _x = $container.offset().left, height = $container.offsetHeight || $container.outerHeight(),
            ...
            var self = this, i18n = $.om.lang.omCalendar,
                data = [i18n.Su, i18n.Mo, i18n.Tu, i18n.We, i18n.Th, i18n.Fr, i18n.Sa], temp = '<span>{$day}</span>',
                offset = this.options.startDay, day_html = '', a = [];
            for (var i = 0; i < 7; i++) {
    ...
    $.extend($.om.omCalendar, {
        Page: function (config, father) {
            ...
            this.createDS = function () {
                var cc = this, fOpts = cc.father.options, s = '',
                    startweekday = (new Date(cc.year + '/' + (cc.month + 1) + '/01').getDay() + fOpts.startDay + 7) % 7,
                    k = cc._getNumOfDays(cc.year, cc.month + 1) + startweekday, i, _td_s;
                var _dis_days = [];
...
(function ($) {
    ...
    $.omWidget('om.omCombo', {
        options: {
            ...
            valueEl.hide();
            this.dropList = $($('<div class="om-widget"><div class="om-widget-content om-droplist"></div></div>').css({
                position: 'absolute',
                zIndex: 2000
            ...
            var self = this, options = self.options, input = self.textInput, valueEl = self.element,
                dropList = self.dropList, expandTrigger = self.expandTrigger, emptyText = options.emptyText;
            ...
            var self = this, options = self.options, inputEl = self.textInput, valueInput = self.element,
                dropList = self.dropList.scrollTop(0).css('height', 'auto'), valuedItem, nowValue = valueInput.val(),
            ...
            var options = this.options;
            $.ajax({
                url: url, method: 'POST', dataType: 'json', success: function (data, textStatus) {
                    self.dataHasLoaded = true;
            ...
            var self = this, valueEl = self.element, dropList = self.dropList, options = self.options,
...
(function ($, undefined) {
    ...
    $.omWidget("om.omDialog", {
        options: {
            ...
            var options = self.options, title = options.title || '&#160;',
                titleId = $.om.omDialog.getTitleId(self.element),
                uiDialog = (self.uiDialog = $('<div></div>')).appendTo(window._top.document.body).hide().addClass(uiDialogClasses + options.dialogClass).css({zIndex: options.zIndex}).attr('tabIndex', -1).css('outline', 0).keydown(
                    function (event) {
                ...
                uiDialogContent = self.element.show().removeAttr('title').addClass('om-dialog-content ' + 'om-widget-content').appendTo(uiDialog),
                uiDialogTitlebar = (self.uiDialogTitlebar = $('<div></div>')).addClass('om-dialog-titlebar ' + 'om-widget-header ' + 'om-corner-all ' + 'om-helper-clearfix').prependTo(uiDialog),
                uiDialogTitlebarClose = $('<a href="#"></a>').addClass('om-dialog-titlebar-close ' + 'om-corner-tr').attr('role', 'button').hover(
                    function () {
                ...
                uiDialogTitlebarCloseText = (self.uiDialogTitlebarCloseText = $('<span></span>')).addClass('om-icon-closethick').text(options.closeText).appendTo(uiDialogTitlebarClose),
                uiDialogTitle = $('<span></span>').addClass('om-dialog-title').attr('id', titleId).html(title).prependTo(uiDialogTitlebar);
            ...
            var self = this, maxZ, thisZ, options = this.options, onBeforeClose = options.onBeforeClose,
            ...
            var self = this, hasButtons = false,
                uiDialogButtonPane = $('<div></div>').addClass('om-dialog-buttonpane ' + 'om-helper-clearfix'),
                uiButtonSet = $("<div></div>").addClass("om-dialog-buttonset").appendTo(uiDialogButtonPane);
            ...
            self.uiDialog.omDraggable({
                ...
                containment: 'window._top.document',
                cursor: 'move',
                onStart: function (ui, event) {
    ...
    $.extend($.om.omDialog, {
        version: "@VERSION", uuid: 0, maxZ: 0, getTitleId: function ($el) {
            var id = $el.attr('id');
            if (!id) {
                this.uuid += 1;
                id = this.uuid;
    ...
    $.extend($.om.omDialog.overlay, {
        ...
        create: function (dialog) {
            ...
            var $el = (this.oldInstances.pop() || $('<div></div>').addClass('om-widget-overlay')).appendTo(window._top.document.body).css({
                width: this.width(),
                height: this.height()
            });
            if ($.fn.bgiframe) {
...
(function ($) {
    ...
    $.omWidget('om.omFileUpload', {
        options: {
            ...
            var id = element.attr('id'), fileId = null, queueId = element.attr('id') + 'Queue',
            ...
            if (settings.onInit() !== false) {
                ...
                swfobject.embedSWF(settings.swf, settings.id + 'Uploader', settings.width, settings.height, '9.0.24', settings.expressInstall, data, {
                    'quality': 'high',
            ...
            element.bind("omFileUploadSelect", {
            ...
            element.bind("omFileUploadProgress", {
...
(function ($) {
    $.omWidget('om.omGrid', {
        options: {
            ...
            _onResizeCallbacks: [],
            onBeforeRowSelect: function (rowIndex, rowData, event) {
                return true;
            },
            ...
            var options = this.options, el = this.element.show().attr({
                cellPadding: 0,
                cellSpacing: 0,
                border: 0
            ...
            var ops = this.options, cms = this._getColModel(), autoExpandColIndex = -1,
                $grid = this.element.closest('.om-grid'), $thead = $('thead', this.hDiv), allColsWidth = 0;
            ...
            var $grid = this.element.closest('.om-grid'), $thead = $('thead', this.hDiv), cms = this._getColModel(),
                ops = this.options;
            if (autoExpandColIndex != -1) {
                ...
                var tableWidth = $grid.width() - 22, usableWidth = tableWidth - $thead.width(),
                    percent = 1 + usableWidth / allColsWidth, toFixedThs = $thead.find('th[axis^="col"] >div');
            ...
            var op = this.options, el = this.element, grid = el.closest('.om-grid'), cms = this._getColModel(),
                allColsWidth = 0, indexWidth = 0, checkboxWidth = 0, autoExpandColIndex = -1;
            thead = $('<thead></thead>');
            ...
            if (op.showIndex) {
                var cell = $('<th></th>').attr({
                    axis: 'indexCol',
                    align: 'center'
                }).addClass('indexCol').append($('<div class="indexheader" style="text-align:center;width:25px;"></div>'));
                tr.append(cell);
                indexWidth = 25;
            }
            if (!op.singleSelect) {
                var cell = $('<th></th>').attr({
                    axis: 'checkboxCol',
                    align: 'center'
                }).addClass('checkboxCol').append($('<div class="checkboxheader" style="text-align:center;width:17px;"><span class="checkbox"/></div>'));
                tr.append(cell);
                checkboxWidth = 17;
            }
            ...
            var el = this.element, op = this.options, grid = el.closest('.om-grid'), pageData = this.pageData,
                nowPage = pageData.nowPage, totalPages = pageData.totalPages, newPage = nowPage;
            ...
            var self = this, el = this.element, grid = el.closest('.om-grid'), op = this.options,
            ...
            var op = this.options, el = this.element, grid = el.closest('.om-grid'), pageStat = $('.pPageStat', grid),
                preProcess = op.preProcess, pageData = this.pageData;
            ...
            var el = this.element, pager = this.pager, pControl = $('.pControl', pager), pageData = this.pageData,
                nowPage = pageData.nowPage, totalPages = pageData.totalPages, data = pageData.data,
                from = op.limit * (nowPage - 1) + 1, to = from - 1 + data.rows.length, pageStat = '';
            if (data.total === 0) {
            ...
            var self = this, el = this.element, op = this.options, grid = el.closest('.om-grid'),
                gridHeaderCols = this._getHeaderCols(), rows = this.pageData.data.rows || [],
                colModel = this._getColModel(), rowClasses = op.rowClasses, tbody = $('tbody', el).empty(),
                isRowClassesFn = (typeof rowClasses === 'function'), pageData = this.pageData,
                start = (pageData.nowPage - 1) * op.limit,
                tdTmp = "<td align='$' abbr='$' class='$'><div align='$' class='innerCol $' style='width:$px'>$</div></td>",
                headerWidth = [], bodyContent = [], cols, j;
            ...
            var el = this.element, op = this.options, tbody = $('tbody', el), tr = this._getTrs().eq(index),
            ...
            var selects = this.getSelections(), $trs = this._getTrs(),
                headerCheckbox = $('th.checkboxCol span.checkbox', this.thead), len = $trs.length;
            headerCheckbox.toggleClass('selected', len > 0 && len == selects.length);
        }, _makeColsResizable: function () {
            var self = this, bDiv = self.tbody.closest('.bDiv'), grid = self.element.closest('.om-grid'),
                $titleDiv = this.titleDiv, differWidth;
            $('th[axis^="col"] div', self.thead).omResizable({
                handles: 'e', containment: 'document', minWidth: 60, resize: function (ui, event) {
                    var callbacks = self.options._onResizableCallbacks;
                    for (var i = 0, len = callbacks.length; i < len; i++) {
                        callbacks[i].call(self);
                    }
                    var _this = $(this), abbr = _this.parent().attr('abbr'),
                        dataCells = $('td[abbr="' + abbr + '"] > div', self.tbody), hDiv = self.thead.closest('.hDiv');
                    ...
                    var callbacks = self.options._onResizableStopCallbacks, $th = $(this).parent(),
...
(function ($) {
    $.omWidget('om.omMenu', {
        options: {contextMenu: false, maxWidth: 200, minWidth: 100, dataSource: 'local'}, show: function (triggerEle) {
            ...
            var self = this;
            $.ajax({
                url: source, method: 'POST', dataType: 'json', success: function (data) {
            ...
            var self = this, element = self.element, uls = element.find("ul"), lis = element.find("li"),
                $doc = $(document), tempEvent;
            for (var i = 0; i < lis.length; i++) {
...
(function ($, undefined) {
    ...
    var _show = function (config) {
        ...
        var mask = $('<div class="om-widget-overlay"/>').appendTo(document.body).show().css({
            height: _height(),
            width: _width()
        });
            ...
            case'confirm':
                ...
                if ($.fn.omButton) {
                    buttonSet.find("button#confirm").omButton({
                        width: 60, onClick: function (event) {
...
(function ($) {
    ...
    $.omWidget("om.omNumberField", {
        options: {
            allowDecimals: true, allowNegative: true, decimalPrecision: 2, disabled: false, onBlur: function (value) {
            }, readOnly: false
        }, _create: function () {
            var options = this.options, self = this;
...
(function ($) {
    $.omWidget('om.omSlider', {
        options: {
            ...
            var _self = this, vars = this._getSliderVars(slider),
                $container = slider.find('ul.om-slider-content:first'), $item = $container.children(),
                opts = this.options, top = 0, left = 0;
            ...
            var _self = this, vars = this._getSliderVars(slider),
                items = slider.find('ul.om-slider-content:first').children(), opts = this.options;
            ...
            var _self = this, vars = this._getSliderVars(slider), opts = this.options,
            ...
            var vars = this._getSliderVars(slider),
                directionNav = $('<div class="om-slider-directionNav">').appendTo(slider), self = this;
            ...
            function _initSlider(slider, startSlide) {
                ...
                var $container = slider.find('ul.om-slider-content:first'), $item = $container.children(), top = 0,
                    left = 0;
...
(function ($) {
    ...
    $.omWidget('om.omSuggestion', {
        options: {
            ...
            var self = this, options = this.options, inputEl = this.element.attr('autocomplete', 'off'),
            ...
            var dropList = this.dropList, index = dropList.find('.' + suggestionHighLightClass).index(),
                all = this._clear();
            index += 1;
            if (index >= all.size()) {
                index = 0;
...
(function () {
    ...
    $.omWidget('om.omTabs', {
        options: {
            ...
            var self = this, $self = this.element, options = this.options, items = this.items,
            ...
            var that = this, $self = this.element, options = this.options,
            ...
            var $anchor = $('<a class="om-tabs-inner"></a>').html(config.title).attr({
                href: '#' + tabId,
                tabId: tabId
            }).css({width: options.tabWidth, height: options.tabHeight}).appendTo($nHeader);
            ...
            var $self = this.element, options = this.options, items = this.items,
                $headers = $self.find('>div.om-tabs-headers'), $panels = $self.find('>div.om-tabs-panels'),
                omtabsHeight = $self.height(), tabId = index, his = this.history;
            ...
            var $headers = this.element.find('>div.om-tabs-headers'), $ul = $headers.children('ul'),
                $lScroller = $ul.prev(), $rScroller = $ul.next(), $li = $ul.children(':last'),
                lBorder = $headers.offset().left, rBorder = $rScroller.offset().left, ulLeft = $ul.offset().left,
...
(function ($) {
    ...
    $.omWidget("om.omTree", {
        ...
        _applyEvents: function (target) {
            var self = this, options = self.options, onClick = options.onClick, onDblClick = options.onDblClick,
                onRightClick = options.onRightClick, onDrag = options.onDrag, onSelect = options.onSelect,
            ...
            if (self.options.draggable) {
                target.omDraggable({
                    revert: "invalid", onStart: function (ui, e) {
                        var node = self.findByNId(ui.helper.attr("id"));
                        onDrag && self._trigger("onDrag", e, node);
                    }
                });
                target.omDroppable({
                    greedy: true, accept: "li.om-tree-node", onDragOver: function (source, event) {
        ...
        _ajaxLoad: function (target, url) {
            var self = this, options = this.options, onBeforeLoad = options.onBeforeLoad, onSuccess = options.onSuccess,
                onError = options.onError;
            onBeforeLoad && self._trigger("onBeforeLoad", null, self.findByNId(target.parent().attr("id")));
            $.ajax({
                url: url, method: 'POST', dataType: 'json', cache: false, success: function (data) {
                    if (options.simpleDataModel) {
        ...
        _toggleCheck: function (target, checked) {
            var checkbox_item = target.find(">div.tree-checkbox"), self = this, options = self.options,
        ...
        select: function (target) {
            var self = this, options = this.options, onBeforeSelect = options.onBeforeSelect,
...
(function ($) {
    $.omWidget("om.omProgressbar", {
        options: {value: 0, text: "{value}%", width: "auto", max: 100}, min: 0, _create: function () {
...
(function ($) {
    $.omWidget('om.omTooltip', {
        options: {
            ...
            var self = this;
            $.ajax({
                url: url, method: 'POST', dataType: 'html', success: function (data, textStatus) {
            ...
            if (event && !options.region) {
                ...
                self.tip.find('.tip-anchor').css({
                    'top': options.anchorOffset + 3,
                    'left': '-9px'
                }).addClass('tip-anchor-left');
            } else {
                ...
                if (options.region == 'right') {
                    ...
                    self.tip.find('.tip-anchor').css({
                        'top': self.tip.outerHeight() - 3,
                        'left': options.anchorOffset + 3
                    });
    ...
    $.omWidget('om.omItemSelector', {
        options: {
            ...
            if (typeof dataSource === 'string') {
                var self = this;
                $.ajax({
                    url: dataSource, method: 'GET', dataType: 'json', success: function (data, textStatus) {
            ...
            var self = this, lp = self.leftPanel, rp = self.rightPanel,
            ...
            var $items = isup ? btn.next() : btn.prev(), $dl = $items.children("dl"), h = $items.outerHeight() - 2,
                dlH = $dl.outerHeight(), dltop = $dl.offset().top, nextbtn, top = this.element.data("dltop") + 20;
            if (isup) {
            ...
            var $items = $('.om-itemselector-items', panel), $dl = $('.om-itemselector-items >dl', panel),
                ...
                itemsH = $items.outerHeight(), dlH = $dl.outerHeight(), dltop = $dl.offset().top,
                top = this.element.data("dltop") + 20;
            ...
            var items = $(".om-itemselector-items", panel), itemsH = items.outerHeight(), $dl = $(">dl", items),
                pageItems = [], hasPageButton = items.next(":visible").length > 0, dtH = $dt.outerHeight(),
                num = itemsH / dtH, dltop = $dl.offset().top, top = this.element.data("dltop");
            ...
            if (arguments.length == 0) {
                var op = this.options, selectedItems = $('.om-itemselector-items>dl>dt', this.rightPanel),
                    returnValue = [];
                selectedItems.each(function () {
...
(function ($) {
    $.omWidget("om.omBorderLayout", {
        options: {
            fit: false, spacing: 5, hideCollapsBtn: false, onBeforeDrag: function (element, event) {
            ...
            var $centerRegion = this._getRegion("center"), $northRegion = this._getRegion("north"),
                $southRegion = this._getRegion("south"), $westRegion = this._getRegion("west"),
                $eastRegion = this._getRegion("east"), $northProxy = this._getRegionProxy("north"),
                $southProxy = this._getRegionProxy("south"), $westProxy = this._getRegionProxy("west"),
                $eastProxy = this._getRegionProxy("east"), northHeight = this._getRegionSize("north").height;
            ...
            if ($southRegion) {
                ...
                $southRegion.css({
                    top: layoutHeight - $southRegion.outerHeight(true),
                    left: westOpt.expandToBottom ? westWidth : 0
                });
                if ($southProxy) {
                ...
                if (panel.resizable && panel.region != "center") {
                    ...
                    $panelEl.parent().omResizable({
                        ...
                        start: function (ui, event) {
                            ...
                            var region = ui.element.attr("region"), maxWidth = $layout.width() - 2 * _self._minWidth,
    ...
    $.omWidget("om.omScrollbar", {
        options: {thick: 8}, _create: function () {
            ...
            $(bars).each(function (index, $bar) {
                ...
                var startPos = 0;
                $bar.omDraggable({
                    axis: type === 'v' ? "y" : "x", containment: $elem, onStart: function (ui, event) {
            ...
            var differ = delta / 120 * 10, $bar = self._vScrollbar, $elem = self.element,
                ...
                barCurTop = $bar.position().top - elemTop, contentHeight = $elem.height(),
                distance = contentHeight - $bar.outerHeight();
            ...
            var $elem = this.element, type = $bar.type, size = 0, offset = $elem.offset(), w = $elem.width(),
                h = $elem.height(), ow = $elem.outerWidth(), oh = $elem.outerHeight(),
                bl = this._getInt($elem.css("border-left-width")), br = this._getInt($elem.css("border-right-width")),
                bt = this._getInt($elem.css("border-top-width")), bb = this._getInt($elem.css("border-bottom-width")),
                pl = this._getInt($elem.css("padding-left")), pt = this._getInt($elem.css("padding-top"));
            if ('v' === type) {
...
(function ($) {
    ...
    $.omWidget.addInitListener('om.omGrid', function () {
        var self = this, cm = this._getColModel(), tds = this._getHeaderCols().filter("[axis^='col']"),
            $pDiv = this.pDiv;
...
(function ($) {
        ...
        $elem.parent().scroll(function () {
            if (self._triggered) {
                ...
                btnScrollTimer = setTimeout(function () {
                    ...
                    self._editView.editBtn.animate({
                        "left": pos.left,
                        "top": pos.top
                    }, self._editView.editing ? "fast" : 0);
                    btnScrollTimer = null;
                }, 300);
            }
        });
        $.extend(this, {
            cancelEdit: function (cancelBtn) {
                ...
                var data = {"update": [], "insert": [], "delete": []}, reqType = type ? type : "update",
                    cData = this._changeData, i;
                if (reqType === "update") {
    ...
    function editRenderer(self) {
        var $tr = $(this), $elem = self.element, $editRow, $editForm, scrollLeft, cm = self._getColModel(), editComp,
            lastValue, ops = self.options;
        if (!_isEditable(self)) {
            return;
        }
        ...
        self._getHeaderCols().each(function (index) {
            ...
            if (!$ins) {
                $wrapper = $("<div style='position:absolute'></div>").css({
                    left: $cell.position().left + scrollLeft,
                    top: 3
                }).appendTo($editForm).addClass("grid-edit-wrapper");
                $ins = editComp.instance = $("<input></input>").attr({
                    "name": compKey,
                    "id": compKey
                }).appendTo($wrapper);
    ...
    function _getEditBtnPosition(self) {
        var $elem = self.element, $bDiv = $elem.parent(), ev = self._editView, $editView = ev.view,
            $editBtn = ev.editBtn, $editRow = ev.editRow, pos = {};
    ...
    function _showEditView(self, tr) {
        var $elem = self.element, $editView = $elem.next(".grid-edit-view"), $editBtn, $editRow,
            position = $(tr).position(), scrollTop = $elem.parent().scrollTop(), ops = self.options;
    ...
    function _resizeView(self, $col, differWidth) {
        var $elem = self.element, view = self._editView, $editView = view.view,
            scrollLeft = $elem.parent().scrollLeft(), $editBtn = view.editBtn, updated;
    ...
    function _saveEditValue(self, tr) {
        var $tr = $(tr), $editRow = self._editView.editRow, comps = self._editComps, rowId = _getRowId($tr),
            index = self._getTrs().index($tr), rowData = self._getRowData(index);
    ...
    function _bindValidation(self) {
        var $editForm = self._editView.editRow.find(">.grid-edit-form"), valiCfg = {}, rules = valiCfg.rules = {},
            messages = valiCfg.messages = {}, colModel = self._getColModel();
                ...
                if (errorList && errorList.length > 0) {
                    $.each(errorList, function (index, obj) {
                        ...
                        if (errorHolder) {
                            ...
                            errorHolder.css({
                                left: docPos.left - tablePos.left + $elem.outerWidth(),
        ...
        $.each(self._editComps, function (name, comp) {
            ...
            if (editor.editable && editor.rules) {
                var key = editor.name || comp.model.name, errorHolder = self._errorHolders[key],
...
(function ($) {
    $.omWidget.addBeforeInitListener('om.omGrid', function () {
        ...
        $.extend(this, {
            _getColModel: function () {
                ...
                var op = this.options, $elem = this.element, $grid = $elem.closest('.om-grid'), cm = op.colModel,
                    allColsWidth = 0, autoExpandColIndex = -1,
                    tmp = "<th class='$' $ $ $ $ $><div class='$' style='text-align:$; $'>$</div></th>",
                    content = ["<thead>"], cols, item, rowHeader, $thead;
    ...
    function _buildBasicColModel(self) {
        self._colModel = [];
        var cm = self._getColModel(), matrix = [], realRowHeader = [], rows = 1, cols = 1, rowHeader, item, colIndex,
            len = cm.length;
        for (var i = 0; i < len; i++) {
            matrix[i] = [];
        }
