...
$(function () {
    ...
    var SheetTableView = BaseTableView.extend({
        getControlHtml: function () { //实现控制区域渲染接口
            var controlstr =
                ...
                "<input type='button' class='button editTrItem' onclick='saveSheetEdit()' style='display:none' value='保存修改'/>";
            if (_top.workflowShowSpx) {
                controlstr += "<br/>&nbsp;&nbsp;<span class='spxItem' style='display:none'>审批项名称：<input class='i_text spxItem' style='width:120px;' data-model='ModelActivityNode' data-property='spxName'/></span>" +
    ...
    $("#save").omButton({icons: {left: RX.handlePath('/medias/style/plat/image/button/accept.gif')}}).click(function () {
    ...
    $("#cancel").omButton({icons: {left: RX.handlePath('/medias/style/plat/image/button/cancel.png')}}).click(function () {
        closeLayer(window);
    });
