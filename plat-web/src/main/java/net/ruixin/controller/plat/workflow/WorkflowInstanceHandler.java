...
public class WorkflowInstanceHandler extends BaseController {
    ...
    public AjaxReturn startWorkflow(String flowCode, Long dataId,
    ...
    public AjaxReturn startWorkflowAndSubmit(String flowCode, Long dataId, String opinion, String title) {
        //启动工作流
        AjaxReturn ar = startWorkflow(flowCode, dataId, title, null);
        //提交第一办理环节任务
        if (ar.getSuccess())
            ar = handleTask(Long.parseLong(((Map) (ar.getData())).get("id").toString()),
                    String.valueOf(super.getCurrentUserId()), null, opinion,
    ...
    public AjaxReturn handleTask(Long id, String userIds, String branch, String opinion, String fj_id,
                                 boolean agree, String autoOpinion, Long dataId, boolean draft) {
        if (null != dataId && draft) {
            //保存草稿后首次提交，更新业务数据ID
            workflowInstanceService.updateWorkflowInstanceData(id, dataId);
        }
        List<Object> param = new ArrayList<>();
        param.add(id);
