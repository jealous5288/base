...
public class WorkflowInstanceDao extends BaseDao<SysWorkflowInstance> implements IWorkflowInstanceDao {
    ...
    public Long getNewestTaskIdByWiId(Long wiId, Long userId) {
        StringBuilder sb = new StringBuilder("SELECT MAX(TI.ID) AS ID\n" +
                "                       FROM SYS_TASK_INSTANCE TI\n" +
                "                      WHERE TI.SFYX_ST = '1'\n" +
                "                        AND TI.WORKFLOW_INSTANCE_ID = ? ");
        List<Long> param = new LinkedList<>();
        param.add(wiId);
        //查找流程实例状态
        String wfInsStatus = jdbcTemplate.queryForObject(
                "SELECT STATUS FROM SYS_WORKFLOW_INSTANCE WHERE ID=? AND SFYX_ST='1'", String.class, wiId);
        if ("5".equals(wfInsStatus)) {//未提交
            if (null != userId) {
                sb.append(" AND TI.USER_ID=? AND TI.IS_FINISH='0' ");
                param.add(userId);
            }
        } else if ("0".equals(wfInsStatus)) {//已完成
            sb.append(" AND TI.IS_FINISH='1' ");
        } else {//运行中
            if (null != userId) {
                sb.append(" AND TI.USER_ID=? AND TI.IS_FINISH='0' ");
                param.add(userId);
        ...
        List<Map<String, Object>> result = jdbcTemplate.queryForList(sb.toString(), param.toArray());
    ...
    @Override
    public void updateWorkflowInstanceData(Long taskId, Long dataId) {
        super.executeSqlUpdate("UPDATE SYS_WORKFLOW_INSTANCE WI\n" +
                "   SET WI.DATA_ID = ?\n" +
                " WHERE WI.SFYX_ST = '1'\n" +
                "   AND WI.ID = (SELECT TI.WORKFLOW_INSTANCE_ID\n" +
                "                  FROM SYS_TASK_INSTANCE TI\n" +
                "                 WHERE TI.ID = ?\n" +
                "                   AND TI.SFYX_ST = '1')", dataId, taskId);
    }
