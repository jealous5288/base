...
public class UserDao extends BaseDao<SysUser> implements IUserDao {
    ...
    public SysUser getUserById(Long userId) {
        ...
        if (user != null) {
            ...
            Hibernate.initialize(user.getSysGlbOrganUserList());
        }
        return user;
    }
    ...
    public FastPagination getUserList(Map map, String hasDelData) {
        ...
        sql.append("SELECT U.ID,\n" +
                ...
                "    ON U.DEFAULT_ORGAN_ID = O.ID  WHERE (U.SFYX_ST = '1' OR U.SFKHF = '1') ");
        if (RxStringUtils.isNotEmpty(map.get("organ_id"))) {
            sql.append("AND U.DEFAULT_ORGAN_ID =? ");
