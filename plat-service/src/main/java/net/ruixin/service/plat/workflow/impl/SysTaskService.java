    ...
    public Map generateTaskHandleJson(Long id, Long userId) {
        ...
        for (SysTask it : list) {
            if (it.getFinish_date() != null && it.getOpinion() != null) {
                ...
                SysOrgan organ = it.getUser_id().getOrganId() != null ? organService.getOrganById(it.getUser_id().getOrganId()) : null;
        ...
        // 任务是否能退回
        Boolean canBackTask = isMe ? isReturned(task) : false;

        Boolean flg; //??
        flg = !status.equals("已办");
        ...
        if (isMe && task.getNode_instance_id().getNode_id().getType() == NodeType.ACTIVITY_NODE
...
//    }
    ...
    public List<Map<String, Object>> findSheetByTask(SysTask task, Boolean flag, String status) {
        ...
        for (SysTaskPageInstance sysTaskPageInstance : taskPageInstanceList) {
            ...
            result.put("spId", resource.getId()); //页面id
